...
** Playing around with chisel
   ...
   After a long while this will run your first test in chisel!
   ...
   Take some time to parse what is going on here, use the print statements to guide you.
   The commented out code is there to show what you should end up with after this section,
   so try to not look too much at it unless your results deviate from what is described.
   
   In its body, the class ~FirstTest~ defines two classes which are the essential parts of the test,
   These are ~MyModule~ which describes a digital circuit, and ~TestRunner~ which describes
   a set of operations to inspect and change the state of the circuit defined by ~MyModule~.
   Lastly, the body of ~FirstTest~ synthesizes an instance of ~MyModule~ which is then subjected
   to the testing procedure defined in ~TestRunner~, however this part is not important to you.
   It should be noted that because ~FirstTest~ extends ~FlatSpec with Matchers~ from the scalatest 
   framework, the class body has access to special syntax such as ~behavior of "my first module"~, 
   which means this is _not_ normal scala syntax that you have to understand.
   First we will look at the module, ~MyModule~
...
*** Creating a peek poke tester
    #+begin_src scala
    class TestRunner(c: MyIncrement) extends PeekPokeTester(c)  {
      for(ii <- 0 until 5){
        poke(c.io.dataIn, ii)
        val o = peek(c.io.dataOut)
